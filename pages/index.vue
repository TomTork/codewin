...
watch(statusSlides, () => {
  if (statusSlides.value === 'success') {
    statusLoading.value += 1;
    slides.value = dataSlides.value ? dataSlides.value.map((it) => {
      return {
        id: it.id,
        name: it.name,
...
watch(statusAdvantages, () => {
  if (statusAdvantages.value === 'success') {
    statusLoading.value += 1;
    advantages.value = dataAdvantages.value ? dataAdvantages.value.map((it) => {
      return {
        id: it.id,
        name: it.name,
...
watch(statusProject, () => {
  if (statusProject.value === 'success') {
    statusLoading.value += 1;
    projects.value = dataProject.value ? dataProject.value : [];
